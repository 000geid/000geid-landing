...
**Cursor:** Upstream mirror of [Dammyjay93/interface-design](https://github.com/Dammyjay93/interface-design). For portfolio/marketing visuals in this repo, pair with Cursor `frontend-design`. Optional deep dives (`references/*.md`) live in the upstream repo only.
...
Native `select` and `datalist` render OS-native elements that cannot be styled. Build custom components — trigger buttons with positioned dropdowns, calendar popovers, styled state management.
